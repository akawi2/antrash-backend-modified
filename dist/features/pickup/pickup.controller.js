"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PickupController = void 0;
const common_1 = require("@nestjs/common");
const pickup_schema_1 = require("../../core/models/pickup.schema");
const pickup_service_1 = require("./pickup.service");
const swagger_1 = require("@nestjs/swagger");
const pickupAdd_dto_1 = require("./dto/pickupAdd.dto");
const addpickup_plan_dto_1 = require("../pickup-plan/dto/addpickup-plan.dto");
let PickupController = class PickupController {
    constructor(pickupService) {
        this.pickupService = pickupService;
    }
    async addPickup(addedPickup) {
        const pickupAdded = this.pickupService.addPickup(addedPickup);
        return pickupAdded;
    }
    async updatePickup(userid, agentid, body) {
        const modified = this.pickupService.signalPickup(userid, agentid, body.check);
        return modified;
    }
    async getAllPicks() {
        return this.pickupService.getAllPickup();
    }
    async addExpressPickup(addPickupPlan) {
        return this.pickupService.addExpressPick(addPickupPlan);
    }
};
__decorate([
    (0, swagger_1.ApiResponse)({ type: pickup_schema_1.Pickup }),
    (0, common_1.Post)('/'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [pickupAdd_dto_1.PickupAdd]),
    __metadata("design:returntype", Promise)
], PickupController.prototype, "addPickup", null);
__decorate([
    (0, swagger_1.ApiResponse)({ type: pickup_schema_1.Pickup }),
    (0, common_1.Patch)('update/:userid;:agentid'),
    __param(0, (0, common_1.Param)('userid')),
    __param(1, (0, common_1.Param)('agentid')),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, Object]),
    __metadata("design:returntype", Promise)
], PickupController.prototype, "updatePickup", null);
__decorate([
    (0, swagger_1.ApiResponse)({ type: pickup_schema_1.Pickup }),
    (0, common_1.Get)('/'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], PickupController.prototype, "getAllPicks", null);
__decorate([
    (0, common_1.Post)('/express'),
    (0, swagger_1.ApiResponse)({ type: pickup_schema_1.Pickup }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [addpickup_plan_dto_1.AddPickupPlan]),
    __metadata("design:returntype", Promise)
], PickupController.prototype, "addExpressPickup", null);
PickupController = __decorate([
    (0, swagger_1.ApiTags)('pickup'),
    (0, common_1.Controller)('pickup'),
    __metadata("design:paramtypes", [pickup_service_1.PickupService])
], PickupController);
exports.PickupController = PickupController;
//# sourceMappingURL=pickup.controller.js.map